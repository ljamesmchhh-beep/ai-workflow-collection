...
- Report the plan item, evidence checked, gap found, files changed, verification run, and whether the original criterion is now satisfied.
...
- Report each milestone criterion as passed, failed, blocked, or not applicable, with the exact evidence command or smoke check used.
...
For dark-themed apps embedding Clerk controls, explicitly verify contrast, compact auth/workspace layout, production-key caveats, and the rendered sign-in/sign-up states in a browser.

For authenticated production dashboards that show persisted extraction/classification results: preserve the user's exact URL/query params, confirm workspace/repo context, verify collection counts and distinct records, open a detail drawer/page, and check console errors.

For public aggregate analytics/social-proof cards, use server-only aggregate APIs, keep privacy guardrails explicit, verify analytics/provider environment assumptions without printing secrets, and check the copy/style in the rendered page.

For approved redesign handoffs: create a dedicated redesign branch, implement tokens/components/pages in order, verify the actual rendered UI in a browser against the design artifact, capture screenshots/screen recording when available, then push the branch and open a PR for human review.

When creating or improving public prompt/docs repositories for a Claude Design → Claude Code redesign workflow: make the two prompts' responsibilities explicit, require users to download/unzip the Claude Design ZIP into the app repo (not rely only on a share link), and document browser visual verification plus branch/PR safety.
